...
import {
  LayoutDashboard, Package, Layers, ShoppingCart, Truck,
  CreditCard, TrendingUp, Users, ClipboardList,
  PlusCircle, History, ShieldCheck, Store,
} from "lucide-react";
  ...
  return (
    <aside className={`sidebar ${collapsed ? "collapsed" : ""}`}>
      ...
      <nav className="sb-nav">
        {!collapsed && (
          <div className="sb-section">{role === "admin" ? "Operations" : "My Store"}</div>
        )}
        {nav.map((n) => {
          const Icon = n.icon;
          const active = pathname === n.href || pathname.startsWith(n.href + "/");
          return (
            <Link key={n.href} href={n.href} className={`nav-item ${active ? "active" : ""}`} title={n.label}>
              <Icon size={18} />
      ...
      <div className="sb-foot">
        <div className="role-chip">
          {role === "admin"
            ? <><ShieldCheck size={15} style={{ color: "var(--fg3)" }} />Admin · Company-wide</>
            : <><Store size={15} style={{ color: "var(--fg3)" }} />{storeName ?? "Branch Manager"}</>
          }
        </div>
      </div>
    </aside>
